...
if __name__ == "__main__":
    ...
    for name in image_names:
        ...
        cv2.imwrite(os.path.join(DEST_DIR,os.path.join(IMAGES,"resized_"+name)),resized_img)

        msk = cv2.imread(os.path.join(DATA_DIR,os.path.join(MASKS,name)))
        resized_msk = cv2.resize(msk, None, fx=0.5, fy=0.5)
        cv2.imwrite(os.path.join(DEST_DIR,os.path.join(MASKS,"resized_"+name)),resized_msk)
    print("Done.")
